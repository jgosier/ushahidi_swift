...
class Reports_Controller extends Main_Controller {
	...
	public function index() 
	{
		...
		// Pagination
		/** #########  Apala required changes ########## DB Call below,  ::factory('incident') */
		...
		// Report Data
		/** #########  Apala required changes ########## DB Call below, */
	...
	public function submit()
	{
		...
		if ($_POST)
		{
			...
			if ($post->validate())
			{
				...
				/** #########  Apala required changes ########## DB Call below,  new Location_Model() */
				...
				/** #########  Apala required changes ########## DB Call below,  new Incident_Model() */
				...
				{
				/** #########  Apala required changes ########## DB Call below,  new Incident_Category_Model() */
				...
				foreach($post->incident_news as $item)
				{
					if(!empty($item))
					{
					/** #########  Apala required changes ########## DB Call below,  new Media_Model() */
				...
				/** #########  Apala required changes ########## DB Call below,  new Incident_Person_Model() */
		...
		{/** #########  Apala required changes ########## DB Call below,  ::factory('incident') */
			...
			if ($_POST)
			{
				...
				if ($post->validate())
				{
					...
					/** #########  Apala required changes ########## DB Call below,  new Comment_Model() */
			...
			if ($id)
			{/** #########  Apala required changes ########## DB Call below,  ::factory('comment') */
		...
		else
		{
			if (!empty($_POST['action']) AND !empty($_POST['type'])) 
			{
				...
				if (!empty($action) AND ($type == 'original' OR $type == 'comment'))
				{
					...
					if ($type == 'original') 
					{/** #########  Apala required changes ########## DB Call below,  ::factory('rate') */
	...
	{
	/** #########  Apala required changes ########## DB Call below,  ::factory('city') */
	...
	private function _get_rating($id = false, $type = NULL)
	{
		if (!empty($id) AND ($type == 'original' OR $type == 'comment'))
		{
			...
			/** #########  Apala required changes ########## DB Call below,  ::factory('ratting') */
	...
	*/
	...
	private function _get_custom_form_fields($incident_id = false, $form_id = 1, $data_only = false) {
	    $fields_array = array();
		 /** #########  Apala required changes ########## DB Call below,  ::factory('form') */
