 ...
 */
 	...
 	/** #########  Apala required changes ##########
		This file handles everything to do with message , sms and email.
		THis needs to be rewrite to use the new API.
		
		*/
 
 
 
  require(APPPATH.'controllers/scheduler/email.php');
...
class Messages_Controller extends Admin_Controller
{
    ...
    */
	function index($service_id = 1)
	{
		...
		//		$emailobj = new Email_Controller();
		//		$emailobj->auto_render=false;
		//		$emailobj->index();
		//		echo " I called Email_controller <br/>";
		//		exit(0);
	
		$this->template->content = new View('admin/messages');

		// Get Title
		/** #########  Apala required changes ########## DB Call below  ::factory('service') */
...
/** #########  Apala required changes ########## DB Call below  ::factory('level') */
    ...
    */
	...
	/** #########  Apala required changes ########## DB Call below  ::factory('message') */				
				...
				if ($reply_to->loaded == true) {
					...
					// Load Users Settings
						/** #########  Apala required changes ########## DB Call below  new Settings_Model(1) */
    ...
    function delete($id = FALSE,$dbtable='message')
    {
        if($dbtable=='twitter'){
	        if ($id){
/** #########  Apala required changes ########## DB Call below ::factory('message'|'twitter'|any passed)*/
...
/** #########  Apala required changes ########## DB Call below  ::factory('message'|'twitter'|any passed)*/        	
    ...
    function deleteMessages($ids,$dbtable='message')
    {
        ...
        if($dbtable=='twitter'){
        	foreach($ids as $id)
	        {
	        	/** #########  Apala required changes ########## DB Call below,  new Twitter_Model(... */
            ...
            foreach($ids as $id)
            {
/** #########  Apala required changes ########## DB Call below,  new Laconica_Model(... */            
    ...
    */
	function twitter()
	{
		...
		//###################################
		// Uncomment this out to get the twitter to come from here.
		//	$this->load_tweets();
		...
		//Order by tweet_hashtag first to bring direct reports to the top
			/** #########  Apala required changes ########## DB Call below  ::factory('twitter') */
...
/** #########  Apala required changes ########## DB Call below,  new Twitter_Model(... */						
    ...
    */
	...
	{ 
/** #########  Apala required changes ########## DB Call below,  new Service_Model(... */
		...
		foreach($tweets as $tweet) {
    		...
    		$reporter = null;
/** #########  Apala required changes ########## DB Call below,  new Reporter_Model(... */
...
/** #########  Apala required changes ########## DB Call below,  new level_Model(... */
...
/** #########  Apala required changes ########## DB Call below,  new Message_Model()*/
	...
	/** #########  Apala required changes ########## DB Call below  ::factory('laconical') */
